...
const Map = ({ spots, itinerary, startPoint, onSetStartPoint }) => {
  ...
  useEffect(() => {
    // Google Maps APIやDirectionsServiceがロードされていない、または出発点か旅程が空の場合はルートをクリアして終了
    if (!window.google || !window.google.maps.DirectionsService || !startPoint || itinerary.length === 0) {
      setDirections(null);
      return;
    }
